...
async def excecute_warn(event, user_id, name, mode, reason="", tt=0, limit=3):
    if mode == "ban":
        ...
        await tbot.edit_permissions(
            event.chat_id, user_id, until_date=None, send_messages=False
        )
        if reason:
            reason = f"\nReason: <i>{reason}</i>"
        await event.respond(
...
async def _(event):
    ...
    if mode == "setwarnlimit":
        if not pattern:
            ...
            await event.edit(
                f"Warn limit settings for {event.chat.title} have been updated to {pattern}."
            )
        else:
...
async def cb_excecute_warn(event, user_id, name, mode, tt=0, limit=3):
    if mode == "ban":
        ...
        await event.edit(
            f'Thats <b>{limit}/{limit}</b> Warnings, <a href="tg://user?id={user_id}">{name}</a> Has been <b>Banned!</b>',
            parse_mode="html",
        )
    elif mode == "kick":
        await tbot.kick_participant(event.chat_id, user_id)
        await event.edit(
            f'Thats <b>{limit}/{limit}</b> Warnings, <a href="tg://user?id={user_id}">{name}</a> has been <b>Kicked!</b>',
            parse_mode="html",
        )
    elif mode == "mute":
        await tbot.edit_permissions(
        ...
        f_t = g_time(tt)
        await event.edit(
            f'Thats <b>{limit}/{limit}</b> Warnings, <a href="tg://user?id={user_id}">{name}</a> has been Muted for <b>{f_t}</b>!',
            parse_mode="html",
        )
        await tbot.edit_permissions(
            event.chat_id,
