...
async def fsub(event):
 ...
 if not channel:
        ...
        if not chat_db:
                await event.reply("Forcesubscribe is currently <b>disabled</b> for this chat.")
        else:
                await event.reply("Forcesubscribe is currently <b>enabled</b>. Users are forced to join <b>@{chat_db.channel}</b> to speak here.", parse_mode='html')
   ...
   try:
     ...
     return await event.reply("❗<b>Invalid channel Username provided.</b>", parse_mode="html")
