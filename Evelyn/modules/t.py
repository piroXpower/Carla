...
import Tedeza.modules.sql.fed_sql as sql
from Tedeza import telethn as tbot
from Tedeza.event import register
from Tedeza.modules.helper_funcs.telethn.chatstatus import is_user_admin as is_admin
...
async def jf(event):
    ...
    if not await is_admin(event, BOT_ID):
        return await event.reply(
            "I need to be admin with permission to restrict users to join a fed!"
        )
...
async def channel_fed(event):
    ...
    buttons = Button.inline("Join fed", data="jfd_{}".format(args)), Button.inline(
        "Cancel", data="cfd"
    )
...
async def leab_fed(event):
    ...
    buttons = Button.inline("Leave fed", data="lfd_{}".format(fed_id)), Button.inline(
        "Cancel", data="cub"
    )
    await event.reply(text, buttons=buttons)
...
async def p(event):
    ...
    if event.is_private:
        return await event.reply(
            "This command is made to be run in a group/channel where the person you would like to promote is present."
        )
    fedowner = sql.get_user_owner_fed_full(event.sender_id)
    if not fedowner:
        return await event.reply(
...
async def fd(event):
    ...
    args, extra = await get_user(event)
    event.chat
    for f in fedowner:
        fed_id = f["fed_id"]
        name = f["fed"]["fname"]
...
async def _(event):
    ...
    if is_user_fed_owner(fed_id, int(r_sender_id)) is True:
        return await event.reply(
            f"I'm not banning the fed owner from their own fed! [{name}]"
        )
...
async def fstat(event):
    ...
    if args:
        if len(args) >= 16:
            ...
            person, extra = await get_user(event)
            user_id = person.id
            replied_user = await tbot(GetFullUserRequest(user_id))
            fname = replied_user.user.first_name
    else:
...
async def ft(event):
    ...
    if fedora:
        return await event.reply(
            f"[{fname}](tg://user?id={user_id}) already owns a federation - they can't own another."
        )
    getuser = sql.search_user_in_fed(fed_id, user_id)
    if not getuser:
        return await event.reply(
            f"[{fname}](tg://user?id={user_id}) isn't an admin in {name} -you can only give your fed to other admins."
        )
    ...
    buttons = [
        Button.inline("Accept", data="fkxd_{}".format(mk)),
        Button.inline("Decline", data="smewxy_{}".format(km)),
    ]
...
async def smex_fed(event):
    ...
    if res:
        sql.user_join_fed(fed_id, int(cname))
...
async def smex(event):
    ...
    if event.sender_id == int(owner):
        ...
        fname = rt.user.first_name
        await event.edit(
            f"[{fname}](tg://user?id={owner}) has cancelled the fed transfer."
        )
        return
    if event.sender_id == int(user):
        rt = await tbot(GetFullUserRequest(int(user)))
        fname = rt.user.first_name
        await event.edit(
            f"[{fname}](tg://user?id={user}) has declined the fed transfer.."
        )
        return
