...
from . import can_change_info, is_admin, get_reply_msg_btns_text, button_parser
...
async def pnotes(event):
 ...
 if not arg:
   ...
   await event.reply("Evelyn will now send a message to your chat with a button redirecting to PM, where the user will receive the note.")
   ...
   await event.reply(f"failed to get boolean value from input: expected one of y/yes/on or n/no/off; got: {arg}")
...
async def nottrig(event):
 ...
 if mode:
  ...
  buttons = Button.url("Click Me", f"t.me/MissEvelyn_bot?start=notes_{event.chat_id}&{name}")
