...
def set_rules(chat_id: int, rule_s: str):
    rules.update_one({"chat_id": chat_id}, {"$set": {"rules": rule_s, "private": False, "button": "Rules"}}, upsert=True)
...
def set_private_rules(chat_id: int, mode):
    ...
    if _rules:
     ...
     rule_s = ""
     button = "Rules"
    rules.update_one({"chat_id": chat_id}, {"$set": {"rules": rule_s, "private": mode, "button": button}}, upsert=True)
