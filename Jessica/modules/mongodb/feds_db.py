...
def new_fed(owner_id: int, fed_id, fedname):
 feds.update_one({"owner_id": owner_id}, {"$set": {"fed_id": fed_id, "fedname": fedname, "fedadmins": [], "flog": None, "chats": []}}, upsert=True)
...
def transfer_fed(owner_id, user_id):
 ...
 if _fed:
  feds.update_one({"fed_id": _fed["fed_id"]}, {"$set": {"owner_id": user_id}}, upsert=True)
...
def user_demote_fed(fed_id, user_id):
 ...
 if _fed:
  ...
  feds.update_one({"fed_id": fed_id}, {"$set": {"fedadmins": fedadmins}}, upsert=True)
