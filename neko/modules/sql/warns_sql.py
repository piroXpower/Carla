...
class WarnSettings(BASE):
    __tablename__ = "warn_settidaj"
    chat_id = Column(String(14), primary_key=True)
    warn_limit = Column(Integer, default=3)
    soft_warn = Column(String, default=False)
    mute_time = Column(Integer, default=3600)

    def __init__(self, chat_id, warn_limit=3, soft_warn="ban", mute_time=3600):
        self.chat_id = str(chat_id)
...
def set_ban_time(chat_id, time):
    with WARN_SETTINGS_LOCK:
        curr_setting = SESSION.query(WarnSettings).get(str(chat_id))
        if not curr_setting:
            curr_setting = WarnSettings(chat_id, mute_time=time)

        curr_setting.mute_time = time
